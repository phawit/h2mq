 ...
 ***********example-homekit2mqtt.json***************
  ...
  "Switch": {
    ...
    "name": "Switch",
    "topic": {
      "setOn": "set/Switch/STATE",
      "statusOn": "status/Switch/STATE"
    },
    "payload": {
      "onTrue": 1,
      "onFalse": 0
...
void loop() {
  if(digitalRead(L1)== HIGH){      client.publish("status/Switch/STATE","1"); }    //Status-MQTT Topic publish
  else{   client.publish("status/Switch/STATE","0");  }    //MQTT Topic publish
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (!client.connected()) {
      if (client.connect("arduinoClient")) {
        ...
        client.subscribe("set/Switch/STATE"); //Set-MQTT Topic description
      }
    }
